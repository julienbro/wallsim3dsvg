...
export class SurfaceTool {
    ...
    activate() {
        this.active = true;
        this.isCreating = false;
        console.log('SurfaceTool activé');
    ...
    deactivate() {
        this.active = false;
        this.isCreating = false;
        console.log('SurfaceTool désactivé');
    }
    
    handleClick(point) {
        if (!this.active) return;
        
        console.log('SurfaceTool: Recherche de contour fermé autour du point', point);
        
        // Trouver toutes les lignes proches du point
        const nearbyLines = this.findNearbyLines(point, 50); // Rayon de recherche de 50cm
        console.log(`${nearbyLines.length} lignes trouvées dans un rayon de 50cm`);
        ...
        console.log(`Contour fermé trouvé avec ${closedContour.length} points`);
    ...
    findNearbyLines(point, radius) {
        ...
        console.log(`${nearbyLines.length} segments trouvés dans un rayon de ${radius}cm`);
        return nearbyLines;
    }
    ...
    findClosedContourAroundPoint(point, segments) {
        ...
        console.log(`Recherche de contour fermé avec ${segments.length} segments, tolérance: ${tolerance}cm`);
        ...
        for (let startSegment of segments) {
            ...
            if (contour && contour.length >= 3) {
                console.log(`Contour candidat trouvé avec ${contour.length} points`);
                
                // Vérifier si ce contour entoure le point
                if (this.isPointInsideContour(point, contour)) {
                    console.log('✓ Contour valide trouvé qui entoure le point');
                    return contour;
                } else {
                    console.log('× Contour trouvé mais ne contient pas le point');
                }
            }
        }
        
        console.log('Aucun contour fermé trouvé');
        return null;
    }
    ...
    buildContourFromSegment(startSegment, allSegments, tolerance) {
        ...
        while (iteration < maxIterations) {
            ...
            if (bestConnection) {
                ...
                if (distanceToStart < tolerance && contour.length >= 3) {
                    console.log(`Contour fermé trouvé avec ${contour.length} points (distance au début: ${distanceToStart.toFixed(2)}cm)`);
                    return contour;
                }
        ...
        if (contour.length >= 3) {
            ...
            if (distanceToStart < tolerance * 2) {
                console.log(`Contour approximativement fermé trouvé avec ${contour.length} points (distance: ${distanceToStart.toFixed(2)}cm)`);
                return contour;
            }
        }
    ...
    createSurfaceFromContour(contour, clickPoint) {
        try {
            console.log(`Création de surface avec contour de ${contour.length} points`);
