...
export class FileManager {
        ...
        object.traverse((child) => {
            if (child.isMesh) {
                ...
                if (child.geometry) {
                    ...
                    const edges = new THREE.EdgesGeometry(child.geometry, 30); // 30 degrés d'angle minimum pour les arêtes
                    ...
                    line.renderOrder = 1; // Pour s'assurer que les lignes sont visibles au-dessus de la géométrie
                    child.add(line);
                } else {
    ...
    async exportViewToPDF() {
        ...
        try {
            ...
            const logoUrl = "https://wallsim3d.com/logo/logofondnoir.png";
    ...
    async importSTLFile() {
        ...
        return new Promise((resolve, reject) => {
            input.onchange = async (event) => {
                ...
                try {
                    document.getElementById('command-output').textContent = 'Chargement du fichier STL...';
                    
                    // Charger le loader STL dynamiquement
                    ...
                    loader.load(fileUrl, (geometry) => {
                        try {
                            this.processSTLGeometry(geometry, file.name);
                            URL.revokeObjectURL(fileUrl); // Nettoyer l'URL temporaire
                            document.getElementById('command-output').textContent = `Fichier STL "${file.name}" importé avec succès`;
                            resolve(geometry);
                        } catch (error) {
                            console.error('Erreur lors du traitement du fichier STL:', error);
                            document.getElementById('command-output').textContent = 'Erreur lors du traitement du fichier STL';
                            reject(error);
                        }
                    }, 
                    (progress) => {
                    ...
                    (error) => {
                        ...
                        document.getElementById('command-output').textContent = 'Erreur lors du chargement du fichier STL';
                        URL.revokeObjectURL(fileUrl);
                        reject(error);
                    });
                } catch (error) {
                    console.error('Erreur lors de l\'initialisation du loader STL:', error);
                    document.getElementById('command-output').textContent = 'Erreur: Loader STL non disponible';
                    reject(error);
                }
            };
    ...
    async importGLTFFile() {
        ...
        return new Promise((resolve, reject) => {
            input.onchange = async (event) => {
                ...
                if (!file) {
                    ...
                    console.log('Chargement du fichier GLB:', file.name);
                    console.log('Taille du fichier:', file.size, 'bytes');
                    console.log('Type MIME du fichier:', file.type);
                    document.getElementById('command-output').textContent = 'Chargement du fichier GLB...';                    const { GLTFLoader } = await import('three/addons/loaders/GLTFLoader.js');
                    console.log('GLTFLoader importé:', typeof GLTFLoader);
                    console.log('GLTFLoader constructor:', GLTFLoader);
                    
                    const loader = new GLTFLoader();
                    console.log('Instance GLTFLoader créée:', loader);
                    console.log('Méthodes disponibles:', Object.getOwnPropertyNames(Object.getPrototypeOf(loader)));                    const fileUrl = URL.createObjectURL(file);
                    console.log('URL temporaire du fichier:', fileUrl);                    // Add a timeout to detect if the loader gets stuck
                    const timeoutId = setTimeout(() => {
                        console.error('TIMEOUT: Le chargement du fichier GLB a pris trop de temps');
                        document.getElementById('command-output').textContent = 'Timeout: Chargement trop long';
                        URL.revokeObjectURL(fileUrl);
                        reject(new Error('Timeout lors du chargement'));
                    }, 30000); // 30 seconds timeout

                    // Try with ArrayBuffer approach first
                    console.log('=== ESSAI AVEC ARRAYBUFFER ===');
                    const reader = new FileReader();
                    reader.onload = (event) => {
                        console.log('Fichier lu en ArrayBuffer, taille:', event.target.result.byteLength);
                        ...
                        loader.parse(
                            ...
                            (gltf) => {
                                ...
                                console.log('=== SUCCESS CALLBACK (PARSE ARRAYBUFFER) EXÉCUTÉ ===');
                                try {
                                    console.log('GLTF parsé avec succès:', gltf);
                                    console.log('Type de gltf:', typeof gltf);
                                    console.log('Propriétés de gltf:', Object.keys(gltf));
                                    
                                    if (!gltf) {
                                        console.error('Erreur: gltf est null ou undefined');
                                        document.getElementById('command-output').textContent = 'Erreur: GLTF null';
                                        reject(new Error('GLTF null'));
                                        return;
                                    }
                                    
                                    if (!gltf.scene) {
                                        console.error('Erreur: gltf.scene est null ou undefined');
                                        console.log('Contenu disponible dans gltf:', Object.keys(gltf));
                                        document.getElementById('command-output').textContent = 'Erreur: Scene GLTF manquante';
                                        reject(new Error('Scene GLTF manquante'));
                                        return;
                                    }
                                    
                                    console.log('Type de gltf.scene:', typeof gltf.scene);
                                    console.log('gltf.scene:', gltf.scene);
                                    console.log('Appel de processGLTFScene avec gltf.scene');
                                    
                                    this.processGLTFScene(gltf.scene);
                                    document.getElementById('command-output').textContent = `Fichier GLB "${file.name}" importé avec succès (ArrayBuffer)`;
                                    resolve(gltf);
                                } catch (error) {
                                    clearTimeout(timeoutId);
                                    console.error('Erreur lors du traitement du fichier GLB (parse ArrayBuffer):', error);
                            ...
                            (error) => {
                                clearTimeout(timeoutId);
                                console.error('=== ERROR CALLBACK (PARSE ARRAYBUFFER) EXÉCUTÉ ===');
                                console.error('Erreur lors du parsing ArrayBuffer:', error);
                                console.error('Type d\'erreur:', typeof error);
                                console.error('Message d\'erreur:', error.message);
                                console.error('Stack trace:', error.stack);
                                document.getElementById('command-output').textContent = 'Erreur lors du parsing du fichier GLB';
                                reject(error);
                            }
                        );
                    };
                    ...
                    reader.onerror = (error) => {
                        ...
                        document.getElementById('command-output').textContent = 'Erreur lors de la lecture du fichier';
                    ...
                    console.error('Erreur lors de l\'initialisation du loader GLB:', error);
                    document.getElementById('command-output').textContent = 'Erreur: Loader GLB non disponible';
                    reject(error);
                }
            };

            input.onerror = () => {
                console.error('Erreur lors de la sélection du fichier');
                reject(new Error('Erreur lors de la sélection du fichier'));
            };
        ...
        // Rotation de 90° sur l'axe X pour aligner correctement le modèle
        scene.rotateX(Math.PI / 2);
        console.log('Rotation appliquée: +90° sur l\'axe X (rouge)');

        // Calculer la boîte englobante pour l'échelle
        const box = new THREE.Box3().setFromObject(scene);
        const size = box.getSize(new THREE.Vector3());
        console.log('Dimensions de la boîte englobante:', size);
        ...
        console.log('Ajout de la scène au modèle');
        this.app.scene.add(scene);
        ...
        scene.traverse((child) => {
            if (child.isMesh && child.geometry) {
                ...
                console.log('  Matériau:', child.material.type);
                console.log('  Visible:', child.visible);
                const meshBox = new THREE.Box3().setFromObject(child);
                const meshSize = meshBox.getSize(new THREE.Vector3());
                console.log('  Dimensions:', meshSize);
                if (!selectableMesh) selectableMesh = child;
            }
        });

        console.log('Ajout de l\'objet à la liste des objets');
        if (typeof this.app.addObject === 'function') {
            this.app.addObject(scene);
        } else if (this.app.objects && Array.isArray(this.app.objects)) {
            this.app.objects.push(scene);
        ...
        console.log('Sélection de l\'objet importé');
        if (selectableMesh) {
